...
namespace BoletoNet
{
    ...
    public class BoletoBancario : System.Web.UI.Control
    {
        ...
        public string MontaHtml(string fileName)
        {
            ...
            C2of5i cb = new C2of5i(Boleto.CodigoBarra.Codigo, 1, 50, Boleto.CodigoBarra.Codigo.Length);
            string base64Image = CreateBase64Image(cb.ToByte());
            string fnCodigoBarras = String.Format("data:image/gif;base64,/9j/{0}", base64Image);
        ...
        private string CreateBase64Image(byte[] imageBytes)
        {
        ...
        public string MontaHtmlEmbedded(bool convertLinhaDigitavelToImage = false)
        {
            ...
            Stream manifestResourceStream = executingAssembly.GetManifestResourceStream("BoletoNet.Imagens." + this.CodigoBanco.ToString("000") + ".jpg");
            string arg = Convert.ToBase64String(new BinaryReader(manifestResourceStream).ReadBytes((int)manifestResourceStream.Length));
            string srcLogo = string.Format("data:image/gif;base64,{0}", arg);
            Stream manifestResourceStream2 = executingAssembly.GetManifestResourceStream("BoletoNet.Imagens.barra.gif");
            string arg2 = Convert.ToBase64String(new BinaryReader(manifestResourceStream2).ReadBytes((int)manifestResourceStream2.Length));
            string srcBarra = string.Format("data:image/gif;base64,{0}", arg2);
            C2of5i c2of5i = new C2of5i(this.Boleto.CodigoBarra.Codigo, 1, 50, this.Boleto.CodigoBarra.Codigo.Length);
            string arg3 = Convert.ToBase64String(c2of5i.ToByte());
            string srcCodigoBarra = string.Format("data:image/gif;base64,{0}", arg3);

            if (convertLinhaDigitavelToImage)
            {
                string text = this.Boleto.CodigoBarra.LinhaDigitavel.Replace("  ", " ").Trim();
                Image image = Utils.DrawText(text, new Font("Arial", 30f, FontStyle.Bold), Color.Black, Color.White);
                string arg4 = Convert.ToBase64String(Utils.ConvertImageToByte(image));
                string str = string.Format("data:image/gif;base64,{0}", arg4);
                this.Boleto.CodigoBarra.LinhaDigitavel = "<img style=\"max-width:420px; margin-bottom: 2px\" src=" + str + " />";
            }
            string text2 = this.HtmlOffLine(null, srcLogo, srcBarra, srcCodigoBarra).ToString();
            if (convertLinhaDigitavelToImage)
            {
                text2 = text2.Replace(".w500", "");
            }
            return text2;
        }
